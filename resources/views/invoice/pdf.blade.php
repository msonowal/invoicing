        ...
        <table>
            ...
            @foreach ($lineItems as $item)
                <tr class="item">
                    ...
                    <td>{{ $invoice->currency }} {{ number_format($item->quantity * $item->unit_price, 2) }}</td>
                </tr>
            @endforeach
            ...
            <tr class="total">
                <td></td>
                <td>
                    Subtotal: {{ $invoice->currency }} {{ number_format($invoice->total_amount / (1 + ($invoice->tax_rate / 100)), 2) }}<br>
                    TAX ({{ $invoice->tax_rate }}%): {{ $invoice->currency }} {{ number_format($invoice->total_amount - ($invoice->total_amount / (1 + ($invoice->tax_rate / 100))), 2) }}<br>
